      ...
      <div className="bg-[#1C1C1F] border border-[#2A2A2E] rounded-2xl max-w-2xl w-full max-h-[92dvh] sm:max-h-[85vh] overflow-hidden shadow-2xl flex flex-col my-auto min-w-0 animate-modal-enter">
        ...
        <div className="p-3 sm:p-4 border-b border-[#2A2A2E] bg-[#121214] space-y-3 shrink-0 overflow-x-hidden">
          ...
          <div className="flex flex-wrap gap-2">
            {[
              { id: 'todos', label: `Todos (${contactsList.length})` },
              { id: 'motos', label: `Motos (${contactsList.filter((c) => c.type === 'moto').length})` },
              { id: 'kitnets', label: `Kitnets (${contactsList.filter((c) => c.type === 'kitnet').length})` },
              { id: 'atrasados', label: `Atrasados (${contactsList.filter((c) => c.status === 'atrasado').length})` },
            ].map((tab) => (
              <button
                key={tab.id}
                type="button"
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${
                  filterType === tab.id
                    ? 'bg-[#8B5CF6] text-[#121214] shadow-xs'
                    : 'bg-[#1C1C1F] text-[#9C9CA3] border border-[#2A2A2E] hover:text-[#F2F1ED] hover:bg-[#2A2A2E]'
                }`}
              >
                {tab.label}
              </button>
              ...
              >
                ...
                <div className="flex items-center gap-2 shrink-0 self-end sm:self-center">
                  <a
                    ...
                    className="p-2.5 rounded-xl bg-[#1C1C1F] hover:bg-[#2A2A2E] border border-[#2A2A2E] text-[#F2F1ED] transition-colors flex items-center gap-1 text-xs font-semibold cursor-pointer"
                    title="Ligar"
                  >
                    <Phone className="w-4 h-4 text-[#9C9CA3]" />
                  ...
                  <button
                    ...
                    className="px-3 py-2.5 rounded-xl bg-[#10B981] hover:bg-[#38b969] text-[#121214] transition-colors flex items-center gap-1.5 text-xs font-bold shadow-xs cursor-pointer"
