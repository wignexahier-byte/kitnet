...
export const GlobalSearchModal: React.FC<GlobalSearchModalProps> = ({
  ...
  const quickFilterChips: Array<{ id: FilterCategory; label: string; count: number; icon: React.ReactNode }> = [
    { id: 'all', label: 'Tudo', count: motos.length + kitnets.length + motoTenants.length + kitnetTenants.length + expenses.length, icon: <Sparkles className="w-3.5 h-3.5 text-[#8B5CF6]" /> },
    { id: 'motos', label: 'Motos', count: motos.length, icon: <Motorbike className="w-3.5 h-3.5 text-[#E07A3F]" /> },
    { id: 'kitnets', label: 'Kitnets', count: kitnets.length, icon: <Home className="w-3.5 h-3.5 text-[#0EA5E9]" /> },
    { id: 'clientes', label: 'Clientes', count: motoTenants.length + kitnetTenants.length, icon: <Users className="w-3.5 h-3.5 text-[#10B981]" /> },
    { id: 'despesas', label: 'Despesas', count: expenses.length, icon: <DollarSign className="w-3.5 h-3.5 text-[#EF4444]" /> },
    { id: 'atrasos', label: 'Em Atraso', count: overduePayments.length, icon: <BadgeAlert className="w-3.5 h-3.5 text-[#EF4444]" /> },
  ];
          ...
          {quickFilterChips.map((chip) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-medium flex items-center gap-1.5 whitespace-nowrap transition-all cursor-pointer shrink-0 border ${
                  isSelected
                    ? 'bg-[#8B5CF6] text-[#121214] border-[#8B5CF6] font-bold shadow-xs'
                    : 'bg-[#1C1C1F] hover:bg-[#2A2A2E] text-[#9C9CA3] hover:text-[#F2F1ED] border-[#2A2A2E]'
                }`}
              >
                {chip.icon}
                <span className="shrink-0">{chip.label}</span>
                <span
                  className={`text-[10px] min-w-[18px] h-4.5 px-1.5 rounded-full font-bold inline-flex items-center justify-center shrink-0 leading-none ${
                    isSelected
                      ? 'bg-[#121214]/20 text-[#121214]'
                      : 'bg-[#121214] text-[#9C9CA3]'
                  }`}
                >
                  {chip.count}
                </span>
